...
class StepperAxis:
    ...
    def nudge(self, delta_deg):
        # Small deterministic tap movement.
        # Base the next tap on the current target so repeated taps stack.
        new_target = self.target_deg() + float(delta_deg)
...
def throttle_update():
    ...
    if throttle_current < throttle_target:
        ...
        set_dac_voltage(
            max(
                throttle_current - THROTTLE_FALL_STEP,
                throttle_target
            )
        )
...
document.getElementById("disarm").addEventListener(
    ...
    function() {
        ...
        // /stop itself performs traction zero + steering hold + brake.
        send("/stop");
        ...
        try:
            ...
            try:
                ...
                # --------------------------------------------
                    ...
                    if armed:
                        last_contact = time.ticks_ms()
                        response(client, "ARMED")
                    else:
                        response(client, "DISARMED")
